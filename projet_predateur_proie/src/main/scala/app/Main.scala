...
object Main extends JFXApp3 {
  ...
  override def start(): Unit = {
    ...
    val (boardWidth, boardHeight) = (600, 600)
    val grid = Grid(boardWidth, boardHeight)
    
    val proieState = ObjectProperty(
      new Proie(Coordinate(grid.width / 4, grid.height / 4), elementRadius, Blue))
    val predateurState = ObjectProperty(
      new Predateur(Coordinate(grid.width / 6, grid.height / 6), elementRadius, Red))
  ...
  def drawContent(
    proieState: ObjectProperty[Proie],
    predateurState: ObjectProperty[Predateur]
  ) = {
  ...
  def updatePositions(
    ...
    if (
      PathFinding.collision_detected(
        proieState.value.position,
        predateurState.value.position,
        proieState.value.rayon.toInt
      )
    ) {
      println("Game Over")
      System.exit(0)
